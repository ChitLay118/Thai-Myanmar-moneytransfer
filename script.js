    ...
    moviesContainer.innerHTML = `
        <div class="max-w-md mx-auto w-full space-y-6">
            ...
            <div class="p-4 bg-gray-800 rounded-lg shadow-lg">
                <h3 class="text-xl font-semibold mb-3 text-cyan-400">အသုံးပြုသူအချက်အလက်</h3>
                
                <div class="space-y-4 mb-6">
                    <div class="flex justify-between items-center">
                        <p class="text-gray-300">အမည်:</p>
                        <p class="font-medium">${currentUser.name || currentUser.email?.split('@')[0] || 'N/A'}</p>
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <p class="text-gray-300">အီးမေးလ်:</p>
                        <p class="font-medium">${currentUser.email || 'N/A'}</p>
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <p class="text-gray-300">User ID:</p>
                        <p class="font-medium text-sm">${currentUser.uid ? currentUser.uid.substring(0, 12) + '...' : 'N/A'}</p>
                    </div>
                    
                    <div class="flex justify-between items-center">
                        <p class="text-gray-300">အကောင့်အမျိုးအစား:</p>
                        <p class="font-medium ${currentUser.role === 'admin' ? 'text-red-400' : 'text-green-400'}">
                ...
                <div class="border-t border-gray-700 pt-4">
                    ...
                    <button onclick="logout()" class="mt-4 w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 rounded transition duration-200 mb-2">
                        အကောင့်မှထွက်ရန်
                    </button>
                    
                    <button onclick="localStorage.clear(); window.location.reload();" class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 rounded transition duration-200">
    ...
    moviesContainer.innerHTML = `
        <div class="admin-panel">
            ...
            <div class="admin-section">
                ...
                <button onclick="window.addVideo()" class="admin-btn mt-2">Video ထည့်ရန်</button>
            </div>
            ...
            <div class="admin-section">
                ...
                <button onclick="window.addAdvertisement()" class="admin-btn mt-2">ကြော်ညာထည့်ရန်</button>
            </div>
...
window.showCustomAlert = function(title, message) {
    // Use the simple alert from auth.js
    if (window.showAlert) {
...
window.closeCustomAlert = function() {
    const modal = document.getElementById('simple-alert');
